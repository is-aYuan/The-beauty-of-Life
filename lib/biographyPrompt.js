...
const {
    getBiographyStyle,
} = require('./biographyStyles');

function buildStyleSection(styleId) {
    const style = getBiographyStyle(styleId);
    return `## 本次回忆录文风：${style.label}
...
function buildBiographyUserPrompt({ accountName, tier, summaries = [], memoryProfile = null, styleId }) {
    ...
    const styleSection = buildStyleSection(styleId);
