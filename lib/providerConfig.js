...
function getProviderConfig(env = process.env) {
    ...
    const chatPrimaryProvider = normalizeProvider(env, 'CHAT_PRIMARY_PROVIDER', LLM_PROVIDERS, llmProvider);
    const defaultFallbackProvider = chatPrimaryProvider === 'doubao' ? 'hunyuan' : 'doubao';
    const chatFallbackProvider = normalizeProvider(env, 'CHAT_FALLBACK_PROVIDER', LLM_PROVIDERS, defaultFallbackProvider);

    return {
        llmProvider,
        voiceProvider,
        chat: {
            primaryProvider: chatPrimaryProvider,
            fallbackProvider: chatFallbackProvider,
            primaryTimeoutMs: readInteger(env, 'CHAT_PRIMARY_TIMEOUT_MS', 15000),
            fallbackTimeoutMs: readInteger(env, 'CHAT_FALLBACK_TIMEOUT_MS', 15000),
            turnTotalTimeoutMs: readInteger(env, 'TURN_TOTAL_TIMEOUT_MS', 40000),
            ttsTimeoutMs: readInteger(env, 'TTS_TIMEOUT_MS', 12000),
        },
        tencent: {
...
function getProviderSummary(config) {
    return {
        ...
        chatPrimaryProvider: config.chat?.primaryProvider || config.llmProvider,
        chatFallbackProvider: config.chat?.fallbackProvider || '(not configured)',
        chatPrimaryTimeoutMs: config.chat?.primaryTimeoutMs,
        chatFallbackTimeoutMs: config.chat?.fallbackTimeoutMs,
        ttsTimeoutMs: config.chat?.ttsTimeoutMs,
