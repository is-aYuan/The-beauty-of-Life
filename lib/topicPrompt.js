...
function buildPersonProfileGuidance(personProfile = {}) {
    ...
    return `基础档案补充：已知基础档案：${knownText}。仍缺：${missingLabels}。如果适合开场或话题自然经过，优先只自然询问「${firstMissing}」，一次只问一个基础信息；老人可以跳过，不要像登记表；不要重复询问已知字段，也不要因为补档案打断当前主题。`;
}
...
- 不要像登记表，不要连续追问多个基础信息。
- 如果基础档案缺失，只按照“基础档案补充”里的一个字段自然询问；用户可以跳过，跳过后继续当前主题。
