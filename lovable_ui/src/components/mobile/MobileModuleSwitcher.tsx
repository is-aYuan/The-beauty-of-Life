...
export function MobileModuleSwitcher<T extends string>({
  ...
  return (
    <header className="shrink-0 border-b border-amber-200 bg-amber-50/95 shadow-sm backdrop-blur">
      <div className="grid grid-cols-[1fr_auto] items-center gap-1.5 px-3 py-1.5">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-10 flex-col items-center justify-center rounded-xl px-1 text-[13px] font-bold transition-colors ${
                  active
                    ? "bg-stone-800 text-amber-50 shadow-sm"
                    : "text-stone-600 hover:bg-amber-100"
                }`}
              >
                <Icon className="mb-0.5 h-[18px] w-[18px]" strokeWidth={2.4} />
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-stone-700 shadow-sm ring-1 ring-amber-100"
        >
