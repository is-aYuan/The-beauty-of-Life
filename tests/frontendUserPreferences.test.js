...
test('builds a root font scale style value from preferences', async () => {
    ...
    assert.equal(buildFontScaleCssValue({ fontSizePreset: 'normal' }), '100%');
    assert.equal(buildFontScaleCssValue({ fontSizePreset: 'large' }), '112%');
    assert.equal(buildFontScaleCssValue({ fontSizePreset: 'extraLarge' }), '125%');
    assert.equal(buildFontScaleCssValue({ fontSizePreset: 'custom', fontScale: 1.18 }), '118%');
});
