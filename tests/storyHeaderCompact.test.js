...
test('story headers share the compact companion stats copy on desktop and mobile', () => {
    ...
    assert.match(source, /const companionStatsText = `今日陪伴 · \$\{userStats\.totalConversations\} 段 · \$\{userStats\.estimatedDurationMin\} 分钟`/);
...
test('story headers use reduced vertical scale for the compact stats treatment', () => {
    ...
    assert.match(source, /shrink-0 border-b border-amber-200 px-4 py-2\.5/);
    assert.match(source, /text-xl font-black leading-tight text-stone-900/);
