...
test('story headers use reduced vertical scale for the compact stats treatment', () => {
    ...
    assert.match(source, /shrink-0 border-b border-amber-200 px-3 py-2/);
    assert.match(source, /xs:px-4/);
    assert.match(source, /flex flex-wrap items-baseline gap-x-2/);
    assert.match(source, /text-xl font-black leading-tight text-stone-900 xs:text-\[22px\]/);
